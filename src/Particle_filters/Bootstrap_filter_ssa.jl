"""
    propegate_ssa_bootstrap!(x_curr::Array{UInt16, 2}, 
...
Propegate n-particles (x) in the bootstrap filter for a SSA (Gillespie) stepper.  
...
"""
function propegate_ssa_bootstrap!(x_curr::Array{UInt16, 2}, 
                                  t_curr::Array{FLOAT, 1},
                                  p::DynModInput, 
                                  model::SsaModel,
                                  h_vec::Array{FLOAT, 1},
    ...
    @inbounds for i in 1:n_particles
        ...
        t_curr[i], iter = step_direct_method!(x_ind, t_start, t_end, model, h_vec, p) 
    end
end

   
"""
    should_break_iter(p::DynModInput, 
                      model::SsaModel,
                      t_vec,
                      h_vec::Array{FLOAT, 1})

Checks if SSA-propgators should be terminated due a high number of iterations. 

Required for computational efficiency. 
"""
function should_break_iter(p::DynModInput, 
                           model::SsaModel,
                           t_vec,
                           h_vec::Array{FLOAT, 1})
    
    x_curr::Array{UInt16, 1} = Array{UInt16, 1}(undef, model.dim)
    model.calc_x0!(x_curr, p)
    t_vec_use = vcat(0.0, t_vec)
    n_data_points::Int64 = length(t_vec)
    iter::Int64 = 0
    t_curr = deepcopy(t_vec[1])

    for i in 1:(n_data_points-1)
        t_curr, iter = step_direct_method!(x_curr, t_curr, t_vec[i+1], model, h_vec, p)
        
        if iter > 0.99e8
            @printf("Will break iteration on bad parameters for Gillespie\n")
            return true
        end
...
"""
    run_filter(filt_opt::BootstrapFilterSsa,
               model_parameters::ModelParameters, 
               random_numbers::RandomNumbers, 
               sde_mod::SdeModel, 
               individual_data::IndData)::FLOAT

Run bootstrap filter for SSA (Gillespie) stepper to obtain unbiased likelihood estimate. 
...
# Args
- `filt_opt`: filter options (BootstrapFilterSsa-struct)
...
- `sde_mod`: underlaying SSA-model for calculating likelihood (SsaModel struct)
...
"""
function run_filter(filter_opt::BootstrapFilterSsa,
                    model_parameters::ModelParameters, 
    ...
    # Check that parameters aren't bad -> to many Gillespie steps 
    if should_break_iter(p, model, t_vec, h_vec)
        return -Inf 
    end
    ...
    if t_vec[1] > 0.0
        ...
        propegate_ssa_bootstrap!(x_curr, t_curr_vec, p, model, h_vec, t_end, n_particles)
    end
